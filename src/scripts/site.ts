...
const savedTheme = localStorage.getItem('bai-theme');
if (savedTheme === 'dark') body.classList.add('dark');

themeToggle?.addEventListener('click', () => {
  body.classList.toggle('dark');
  localStorage.setItem('bai-theme', body.classList.contains('dark') ? 'dark' : 'light');
  themeToggle.setAttribute('aria-label', body.classList.contains('dark') ? '切换浅色模式' : '切换深色模式');
});
