...
const progressLabel = document.querySelector<HTMLElement>('#chicken-progress-label');
const progressValue = document.querySelector<HTMLElement>('#chicken-progress-value');
const progressBar = document.querySelector<HTMLElement>('#chicken-progress-bar');
...
const formatPercent = (value: number, total: number) => `${(value / total * 100).toFixed(3)}%`;

const runSimulation = async () => {
  if (!countInput) return;
  const total = Math.max(10, Math.min(1_000_000, Math.floor(Number(countInput.value) || 100_000)));
  ...
  for (let operation = 0; operation < total; operation += 1) {
    ...
    if (Math.random() * available < twoLegs) { twoLegs -= 1; oneLeg += 1; } else { oneLeg -= 1; }
    if (operation % chunk === 0 || operation === total - 1) {
      const percent = Math.round((operation + 1) / total * 100);
      setProgress(percent, `第 ${formatNumber(operation + 1)} / ${formatNumber(total)} 次操作`);
  ...
  if (insight) insight.textContent = `本次模拟中，${formatNumber(twoLegs)} 只完好鸡与 ${formatNumber(zeroLegs)} 只零腿鸡互相接近；理论上的大样本完好鸡比例约为 31.7844%。`;
  if (results) results.hidden = false;
  if (statusMessage) statusMessage.textContent = '模拟完成，可以修改数量后再次运行。';
  runButton?.removeAttribute('disabled');
};
const reset = () => { if (countInput) countInput.value = '100000'; if (results) results.hidden = true; if (statusMessage) statusMessage.textContent = '设置数量后运行一次模拟'; setProgress(0, '等待开始'); };
