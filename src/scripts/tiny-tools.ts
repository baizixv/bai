...
export {};
type ToolName = 'timestamp' | 'base64' | 'hash' | 'color' | 'random-color' | 'json' | 'qr';
...
const configs: Record<ToolName, [string, string, string, string, string]> = {
  ...
  'random-color': ['COLOR GENERATOR', '随机颜色生成', '点击转换生成颜色', '颜色值', '每次生成一个随机颜色，同时显示 HEX 和 RGB。'],
  qr: ['QR GENERATOR', '二维码生成', '输入文字或链接', '二维码信息', '二维码在浏览器本地生成，可以下载到本地。']
};
...
const randomColor = () => { const hex = `#${Math.floor(Math.random() * 0xffffff).toString(16).padStart(6, '0')}`; const rgb = [1, 3, 5].map((index) => parseInt(hex.slice(index, index + 2), 16)); return `${hex}\nRGB(${rgb.join(', ')})`; };
...
const run = (name: ToolName, value: string) => { if(name==='timestamp'){const n=value.trim()?Number(value):Date.now(); if(!Number.isFinite(n)) throw Error('请输入有效的时间戳'); const d=new Date(n); if(Number.isNaN(d.valueOf())) throw Error('时间戳超出范围'); return `${d.toLocaleString('zh-CN')}\nISO：${d.toISOString()}\nUnix 秒：${Math.floor(n/1000)}`;} if(name==='base64') { try{return base64Decode(value.trim())} catch{return base64Encode(value)} } if(name==='hash') return md5(value); if(name==='color') return convertColor(value); if(name==='random-color') return randomColor(); if(name==='qr') return value.trim() || '请输入文字或链接'; return JSON.stringify(JSON.parse(value),null,2); };
const requestedTool = new URLSearchParams(window.location.search).get('tool');
let current: ToolName = requestedTool === 'base64' || requestedTool === 'hash' || requestedTool === 'color' || requestedTool === 'random-color' || requestedTool === 'json' || requestedTool === 'qr' ? requestedTool : 'timestamp';
const update = () => { const c=configs[current]; [kicker,title,inputLabel,outputLabel,note].forEach((el,i)=>{if(el)el.textContent=c[i]}); if(input)input.placeholder=c[2]; if(output)output.placeholder=c[3]; if(qrOutput)qrOutput.hidden=true; if(qrDownload)qrDownload.hidden=true; if(colorSwatch)colorSwatch.hidden=true; };
...
document.querySelector('#tool-run')?.addEventListener('click',async()=>{try{const value=input?.value??''; if(output)output.value=run(current,value); if(current==='qr' && value.trim() && qrOutput){ const dataUrl=await QRCode.toDataURL(value.trim(),{width:240,margin:2,errorCorrectionLevel:'M'}); qrOutput.innerHTML=`<img src="${dataUrl}" alt="二维码" />`; qrOutput.hidden=false; if(qrDownload){qrDownload.hidden=false;qrDownload.onclick=()=>{const link=document.createElement('a');link.href=dataUrl;link.download='bai-qr-code.png';link.click();};} } if(current==='random-color' && colorSwatch && output){colorSwatch.style.background=output.value.split('\\n')[0];colorSwatch.hidden=false;} }catch(error){if(output)output.value=error instanceof Error?error.message:'处理失败'}});
...
document.querySelector('#tool-clear')?.addEventListener('click',()=>{if(input)input.value='';if(output)output.value='';if(qrOutput)qrOutput.hidden=true;if(qrDownload)qrDownload.hidden=true;if(colorSwatch)colorSwatch.hidden=true;}); update();
