...
const favorites = new Set<string>(JSON.parse(localStorage.getItem('ogden-favorites') || '[]'));
const progress = new Set<string>(JSON.parse(localStorage.getItem('ogden-progress') || '[]'));

const save = () => {
  localStorage.setItem('ogden-favorites', JSON.stringify([...favorites]));
  localStorage.setItem('ogden-progress', JSON.stringify([...progress]));
};
const escapeHtml = (value: string) => value.replace(/[&<>"']/g, (char) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' }[char] || char));
const current = () => filtered[selected % Math.max(filtered.length, 1)];
const categoryCount = (key: string) => key === 'all' ? allWords.length : allWords.filter((word) => word.category === key).length;

const updateCategoryCounts = () => {
  document.querySelectorAll<HTMLElement>('.ogden-category').forEach((button) => {
    const span = button.querySelector('span');
    if (span) span.textContent = String(categoryCount(button.dataset.category || 'all'));
  });
};
...
const browse = () => {
  ...
  if (stageLabel) stageLabel.textContent = category === 'all' ? '850 WORDS' : category.toUpperCase();
  if (content) content.innerHTML = filtered.length ? `<div class="ogden-word-grid">${filtered.slice(0, 18).map(wordCard).join('')}</div>` : '<div class="ogden-empty">没有找到匹配的词。换个关键词试试。</div>';
};

const practice = () => {
  const word = current();
  if (!word || !content) return;
  if (stageCount) stageCount.textContent = `${selected + 1} / ${filtered.length}`;
  ...
  if (mode === 'flashcard') {
    ...
    const options = [word, ...allWords.filter((item) => item.id !== word.id).sort(() => Math.random() - 0.5).slice(0, 3)].sort(() => Math.random() - 0.5);
...
const applyFilters = () => {
  ...
  filtered = allWords.filter((word) => (category === 'all' || word.category === category) && (!query || word.word.toLowerCase().includes(query) || word.chinese.includes(query) || (word.phonetic || '').includes(query)));
  selected = 0;
  render();
};

document.querySelectorAll<HTMLButtonElement>('.ogden-category').forEach((button) => button.addEventListener('click', () => {
  category = button.dataset.category || 'all';
  document.querySelector('.ogden-category.active')?.classList.remove('active');
  button.classList.add('active');
  applyFilters();
}));
document.querySelectorAll<HTMLButtonElement>('.ogden-mode').forEach((button) => button.addEventListener('click', () => {
...
content?.addEventListener('click', (event) => {
  ...
  if (favorite) { const id = favorite.dataset.favorite!; favorites.has(id) ? favorites.delete(id) : favorites.add(id); save(); render(); return; }
  const speak = target.closest<HTMLElement>('[data-speak]')?.dataset.speak;
  if (speak && 'speechSynthesis' in window) window.speechSynthesis.speak(new SpeechSynthesisUtterance(speak));
  const answer = target.closest<HTMLElement>('[data-answer]');
  if (answer) { const feedback = document.querySelector<HTMLElement>('#ogden-feedback'); const correct = answer.dataset.answer === current()?.id; if (feedback) feedback.textContent = correct ? '正确。' : `再想想，${current()?.chinese}`; if (correct) progress.add(current().id); save(); }
  if (target.id === 'practice-reveal') { if (showAnswer) { selected = (selected + 1) % filtered.length; showAnswer = false; } else showAnswer = true; render(); }
  if (target.id === 'spelling-check') { const value = document.querySelector<HTMLInputElement>('#spelling-input')?.value.trim().toLowerCase(); const feedback = document.querySelector<HTMLElement>('#ogden-feedback'); if (value === current()?.word.toLowerCase()) { if (feedback) feedback.textContent = '正确。'; progress.add(current().id); selected = (selected + 1) % filtered.length; spellingAnswer = ''; } else { if (feedback) feedback.textContent = `答案是 ${current()?.word}`; spellingAnswer = value || ''; } save(); }
  if (target.id === 'expression-check') { const value = document.querySelector<HTMLTextAreaElement>('#expression-input')?.value.trim(); const feedback = document.querySelector<HTMLElement>('#ogden-feedback'); if (value) { progress.add(current().id); if (feedback) feedback.textContent = '完成。继续用下一个词表达。'; save(); } else if (feedback) feedback.textContent = '先写下一句，再完成练习。'; }
});
...
const start = async () => {
  try {
    ...
    if (content) content.innerHTML = '<div class="ogden-empty">词汇数据加载失败，请刷新页面重试。</div>';
  }
};
start();
